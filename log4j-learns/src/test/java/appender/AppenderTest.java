...
public class AppenderTest {
     ...
     *
     * {@link JDBCAppender}JDBCAppender将日志保存到数据库的表中，由于数据库保存操作是一个比较费时的
     ...
     *
     * {@link JMSAppender}JMSAppender类将LoggingEvent实例序列化成ObjectMessage，并将其发送到
     ...
     *
     * {@link TelnetAppender}TelnetAppender类将日志消息发送到指定的Socket端口（默认为23），用户可以
     ...
     *
     * {@link SMTPAppender}SMTPAppender将日志消息以邮件的形式发送出来，默认实现，它会先缓存日志信息，
     * 只有当遇到日志级别是ERROR或ERROR以上的日志消息时才通过邮件的形式发送出来，如果在遇到触发发送的日志发生之前缓存中的日志信息
     * 已满，则最早的日志信息会被覆盖。用户可以通过setEvaluatorClass()方法改变触发发送日志的条件。
     *
     * {@link SocketAppender}SocketAppender将日志消息（LoggingEvent序列化实例）发送到指定Host的
     ...
     *
     * {@link LF5Appender}将日志显示在Swing窗口中。对Swing不熟，没怎么看代码，不过可以使用一下测试用例
     * 简单的做一些测试，提供一些感觉。
     *
